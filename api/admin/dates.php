...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['dates']) && !empty($_POST['dates'])) {
            error_log('✅ FormData 방식 감지');
            error_log('$_POST[dates]: ' . substr($_POST['dates'], 0, 200));
            $dates = json_decode($_POST['dates'], true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log('❌ FormData JSON 파싱 오류: ' . json_last_error_msg());
                jsonResponse([
                    'success' => false,
                    'message' => 'FormData JSON 파싱 오류: ' . json_last_error_msg()
                ], 400);
            }
            error_log('✅ FormData 파싱 성공, 날짜 개수: ' . count($dates));
        }
        // 2. JSON 방식 확인
        else {
            ...
            if (!isset($data['dates'])) {
                error_log('❌ dates 키가 없음. 받은 데이터: ' . json_encode($data));
                jsonResponse([
                    'success' => false,
                    'message' => 'dates 필드가 필요합니다.'
                ], 400);
            }
            
            $dates = $data['dates'];
            error_log('✅ JSON 파싱 성공, 날짜 개수: ' . count($dates));
        }
        
        if (!is_array($dates)) {
            error_log('❌ dates가 배열이 아님: ' . gettype($dates));
            jsonResponse([
                'success' => false,
                'message' => '날짜 목록은 배열이어야 합니다.'
            ], 400);
        }
        ...
        if ($result === false) {
            error_log('❌ 파일 쓰기 실패: ' . $datesFile);
            jsonResponse([
                'success' => false,
                'message' => '파일 저장에 실패했습니다. 권한을 확인해주세요.'
            ], 500);
        }
        
        error_log('✅ 날짜 저장 성공: ' . $result . ' bytes');
        
        jsonResponse([
            'success' => true,
            'message' => '날짜 목록이 저장되었습니다.'
        ], 200);
        
    } catch (Exception $e) {
        error_log('❌ 날짜 목록 저장 예외: ' . $e->getMessage());
